...
// Ambil semua soal untuk Bahasa
$stmt = $pdo->prepare("SELECT s.soal_id, s.teks_soal, s.option_a, s.option_b, s.option_c, s.correct_answer, s.gambar_path FROM SoalKuis s JOIN Kuis k ON s.kuis_id = k.kuis_id WHERE k.nama_kuis = 'Bahasa'");
$stmt->execute();
$soal_bahasa = $stmt->fetchAll();

// Proses tambah soal
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_soal'])) {
    $kuis_id = 1; // Kuis_id untuk Bahasa
    ...
    header("Location: admin-soal-bahasa.php");
    exit;
}
...
<head>
    ...
    <title>Pintar Ceria - Kelola Soal Bahasa</title>
...
<body>
    ...
    <main>
        ...
        <div class="section-card">
            <div class="section-title">Daftar Soal Bahasa</div>
            <ul>
                <?php foreach ($soal_bahasa as $s): ?>
                    <li class="soal-item">
                ...
                <?php endforeach;
                if (empty($soal_bahasa)) echo "<li class='no-content'>Belum ada soal untuk Bahasa.</li>";
                ?>
            </ul>
        </div>
