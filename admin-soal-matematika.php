...
// Ambil semua soal untuk Matematika
$stmt = $pdo->prepare("SELECT s.soal_id, s.teks_soal, s.option_a, s.option_b, s.option_c, s.correct_answer, s.gambar_path FROM SoalKuis s JOIN Kuis k ON s.kuis_id = k.kuis_id WHERE k.nama_kuis = 'Matematika'");
$stmt->execute();
$soal_matematika = $stmt->fetchAll();

// Proses tambah soal
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_soal'])) {
    $kuis_id = 2; // Kuis_id untuk Matematika (sesuaikan dengan ID di tabel Kuis)
    ...
    header("Location: admin-soal-matematika.php");
    exit;
}
...
<head>
    ...
    <title>Pintar Ceria - Kelola Soal Matematika</title>
...
<body>
    ...
    <main>
        <!-- Daftar Soal Matematika -->
        <div class="section-card">
            <div class="section-title">Daftar Soal Matematika</div>
            <ul>
                <?php foreach ($soal_matematika as $s): ?>
                    <li class="soal-item">
                        ...
                        <div class="action-links">
                            ...
                            <form method="POST" onsubmit="return confirm('Yakin hapus soal ini?')" class="inline">
                                ...
                                <button type="submit" name="_soal" class="text-red-500">Hapus</button>
                            </form>
                        </div>
                    </li>
                <?php endforeach;
                if (empty($soal_matematika)) echo "<li class='no-content'>Belum ada soal untuk Matematika.</li>";
                ?>
            </ul>
        </div>
